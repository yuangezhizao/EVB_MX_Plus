...
static void light_power_on(void)
{
    ...
    sg_ProductData.m_light_switch = 1;
...
static void motor_power_on(void)
{
    ...
    sg_ProductData.m_motor_switch = 1;
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    if (sg_ProductData.m_light_switch == 1) {
        light_power_on();
    ...
    if (sg_ProductData.m_motor_switch == 1){
        motor_power_on();
    } else{
        motor_power_off();
    }
...
uint8_t get_soil_humi(void)
{
    ...
    // 200423	
    OLED_ShowNum(0, 0, advalue, 4, 16);
	char *str = "mV";
    OLED_ShowString(32, 0, (uint8_t*)str, 16);

    humi = (4000 - advalue) / 30;

    OLED_ShowNum(0, 2, humi, 2, 16);
    str = "%";
    OLED_ShowString(16, 2, (uint8_t*)str, 16);
    return humi;
}
...
int deal_up_stream_user_logic(DeviceProperty *pReportDataList[], int *pCount)
{
    ...
    sg_ProductData.m_soil_humi = _humi;
    sg_DataTemplate[0].state = eCHANGED;

    float _light;
    _light = Convert_BH1750();

    sg_ProductData.m_brightness = _light;
    sg_DataTemplate[1].state = eCHANGED;

    DHT11_Data_TypeDef DHT11_Data;
    if( DHT11_Read_TempAndHumidity ( & DHT11_Data ) == SUCCESS) {	
        printf("dht11: humidity: %f; temperature %f \n", DHT11_Data.humidity,DHT11_Data.temperature);
        sg_ProductData.m_temperature = DHT11_Data.temperature;
        sg_DataTemplate[2].state = eCHANGED;

        sg_ProductData.m_humidity = DHT11_Data.humidity;
